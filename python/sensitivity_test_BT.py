...
data_change = data_change.add_suffix('_change')
 ...
 # Logit transformation: The logit function is defined as logit(p) = log(p/(1-p)). Note that logit(0) = -inf, logit(1) = inf, and logit(p) for p<0 or p>1 yields nan. applied on dependent variables only data_logit = pd.DataFrame(columns = pd.Series((data_dep.loc[:,data_dep.columns != 'Date']).add_suffix('_logit').columns))
...
for i in range (0, len(model_list_BPV)):
    Y_BPVinc = pd.DataFrame( 
        model_list_BPV.loc[
            i, 
            list(model_list_BPV.columns[(model_list_BPV.columns).str.startswith('Y_pred_BPVinc')==True])] )
    ...
    # Now concatenate with the existing Y_BPVinc DataFrame
    Y_BPVinc.columns = [0]
...
for i in range (0, len(model_list_BPV)):
    Y_BPVdec = pd.DataFrame( 
        model_list_BPV.loc[
            i, list(
                model_list_BPV.columns[(model_list_BPV.columns).str.startswith('Y_pred_BPVdec')==True])] )
