# -*- coding: utf-8 -*-
"""
Created on Wed Mar  2 22:19:32 2022

@author: QE495FS
"""
#conda install -c conda-forge modin

# clear all variables from Python environment before start running the tool
# %reset -f
...
import json
from statsmodels.tsa.ar_model import AutoReg  # for Model Reidual Error
...
# H1: Hypothesis 1 if no model resulted from HOs
#p_value_model = 0.15
...
check_list_macro=[]
check_list_macro=[('xGDP','xCON'),('xCON','xGDP'),  # gdp with private consumption
       ('xGDP','xGFC'), ('xGFC','xGDP'), # gdp with investments
       ('xGDP','xUNE'),('xUNE','xGDP'), # gdp with unemployment
       ('xGFC','xCON'),('xCON','xGFC'), # investment with private consumption
       ('xUNE','xGFC'),('xGFC','xUNE'), # investment with unemployment
       ('xUNE','xCON'), ('xCON','xUNE')] # unemployment with private consumption
...
try:
    ...
    dataset = pd.concat([pd.DataFrame(dataset['Date']),
        ...
        pd.DataFrame(dataset.loc[:,dataset.columns[pd.Series(dataset.columns).isin(extParams['modelVars'])]])], axis=1)
    ...
    dataset = pd.concat([dataset, data_diff, data_diff2, data_logit, data_logitdiff, data_change, data_sqrt, data_reciprocal,data_cuberoot], axis=1)
    ...
    #     df = pd.concat([pd.DataFrame(adfuller(independent_var.iloc[:,i], autolag = 'AIC'))[0:4], pd.DataFrame(list(adfuller(independent_var.iloc[:,i], autolag = 'AIC')[4].items()))[1]], ignore_index=True)
    #     df = df.T
    #     ADF_output = pd.concat([ADF_output, df], ignore_index=True)
    # ADF_output = ADF_output.set_axis(['ADF_Statistic','ADF_Pvalue','ADF_lags_used','ADF_observation_used','ADF_Critical_Value_1%', 'ADF_Critical_Value_5%','ADF_Critical_Value_10%'], axis=1, inplace = False)
    # ADF_output.insert(loc = 0, column = 'VAR_name', value = pd.Series(independent_var.columns))
    # ADF_output['ADF'] = np.where( ((ADF_output['ADF_Pvalue']<=p_value_blue) & (ADF_output['ADF_Statistic']<=ADF_output['ADF_Critical_Value_5%'])),
    #     True, False)
    ...
    # for i in range (0,len(independent_var.columns)):
    #     df = pd.concat([pd.DataFrame(kpss(independent_var.iloc[:,i], regression='ct', nlags = "auto"))[0:3], pd.DataFrame(list(kpss(independent_var.iloc[:,i], regression='ct', nlags = "auto")[3].items()))[1]], ignore_index=True)
    #     df = df.T
    ...
    # for i in range (0,len(independent_var.columns)):
    #     df = pd.concat([pd.DataFrame(data=PhillipsPerron(independent_var.iloc[:,i],trend='ct',test_type='rho').stat,index=["row1"], columns=["PP_Statistic"]),
    #                     pd.DataFrame(data=PhillipsPerron(independent_var.iloc[:,i],trend='ct',test_type='rho').pvalue,index=["row1"], columns=["PP_Pvalue"]),
    #                     pd.DataFrame(data=PhillipsPerron(independent_var.iloc[:,i],trend='ct',test_type='rho').lags,index=["row1"], columns=["PP_Lags"])],axis=1, ignore_index=True)
    #     #df = df.T
    #     PP_output = pd.concat([PP_output, df], ignore_index=True)
    # PP_output = PP_output.set_axis(['PP_Statistic','PP_Pvalue','PP_Lags'], axis=1, inplace = False)
    # PP_output.insert(loc = 0, column = 'VAR_name', value = pd.Series(independent_var.columns))
    # PP_output['PP'] = np.where( (PP_output['PP_Pvalue']<=p_value_blue), True, False)
    ...
    #     df = pd.concat([pd.DataFrame(adfuller(dependent_var.iloc[:,i], autolag = 'AIC'))[0:4], pd.DataFrame(list(adfuller(dependent_var.iloc[:,i], autolag = 'AIC')[4].items()))[1]], ignore_index=True)
    #     df = df.T
    #     ADF_output = pd.concat([ADF_output, df], ignore_index=True)
    # ADF_output = ADF_output.set_axis(['ADF_Statistic','ADF_Pvalue','ADF_lags_used','ADF_observation_used','ADF_Critical_Value_1%', 'ADF_Critical_Value_5%','ADF_Critical_Value_10%'], axis=1, inplace = False)
    ...
    # KPSS_output = pd.DataFrame(columns = [0, 1, 2, 3, 4, 5, 6])
    # for i in range (0,len(dependent_var.columns)):
    #     df = pd.concat([pd.DataFrame(kpss(dependent_var.iloc[:,i], regression='ct', nlags = "auto"))[0:3], pd.DataFrame(list(kpss(dependent_var.iloc[:,i], regression='ct', nlags = "auto")[3].items()))[1]], ignore_index=True)
    #     df = df.T
    #     KPSS_output = pd.concat([KPSS_output, df], ignore_index=True)
    # KPSS_output = KPSS_output.set_axis(['KPSS_Statistic','KPSS_Pvalue','KPSS_lags_used','KPSS_Critical_Value_10%', 'KPSS_Critical_Value_5%','KPSS_Critical_Value_2.5%', 'KPSS_Critical_Value_1%'], axis=1, inplace = False)
    ...
    # PP_output = pd.DataFrame(columns = [0, 1, 2])
    # for i in range (0,len(dependent_var.columns)):
    #     df = pd.concat([pd.DataFrame(data=PhillipsPerron(dependent_var.iloc[:,i],trend='ct',test_type='rho').stat,index=["row1"], columns=["PP_Statistic"]),
    #                     pd.DataFrame(data=PhillipsPerron(dependent_var.iloc[:,i],trend='ct',test_type='rho').pvalue,index=["row1"], columns=["PP_Pvalue"]),
    #                     pd.DataFrame(data=PhillipsPerron(dependent_var.iloc[:,i],trend='ct',test_type='rho').lags,index=["row1"], columns=["PP_Lags"])],axis=1, ignore_index=True)
    #     #df = df.T
    #     PP_output = pd.concat([PP_output, df], ignore_index=True)
    # PP_output = PP_output.set_axis(['PP_Statistic','PP_Pvalue','PP_Lags'], axis=1, inplace = False)
    ...
    for i in range(0,len(df)):
        ...
        for d in range(0,len(comb_full)):
            if ((len(set(w[0:3] for w in comb_full[d]))==len(comb_full[d])) and not(any(item in check_list_macro for item in list(combinations(set(w[0:4] for w in comb_full[d]),2)))))==True:
                # and (check_gdp_prcons not in list(combinations(set(w[0:4] for w in comb_full[d]),2))) and (check_prcons_gdp not in list(combinations(set(w[0:4] for w in comb_full[d]),2))) and (check_gdp_fiscal not in list(combinations(set(w[0:4] for w in comb_full[d]),2))) and (check_fiscal_gdp not in list(combinations(set(w[0:4] for w in comb_full[d]),2))) and (check_gdp_invest not in list(combinations(set(w[0:4] for w in comb_full[d]),2))) and (check_invest_gdp not in list(combinations(set(w[0:4] for w in comb_full[d]),2))) and (check_fiscal_prcons not in list(combinations(set(w[0:4] for w in comb_full[d]),2))) and (check_prcons_fiscal not in list(combinations(set(w[0:4] for w in comb_full[d]),2))) and (check_invest_prcons not in list(combinations(set(w[0:4] for w in comb_full[d]),2))) and (check_prcons_invest not in list(combinations(set(w[0:4] for w in comb_full[d]),2))) and (check_invest_fiscal not in list(combinations(set(w[0:4] for w in comb_full[d]),2))) and (check_fiscal_invest not in list(combinations(set(w[0:4] for w in comb_full[d]),2)))
                comb_temp = list(comb_full[d])
                comb.append(comb_temp)
        
        inputs = range (0,len(comb))
        try:
            model_results=pd.concat(Parallel(n_jobs=extParams['threads'], prefer="threads", verbose=5)(delayed(processInput)(k) for k in inputs), axis=0, ignore_index=True)
    ...
    model_list_final_subset = model_list_final_subset.loc[
        (model_list_final_subset[ pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xGDP')==True)]) ].max(axis=1) < 0) |
    ...
    model_list_final_subset = model_list_final_subset.loc[
        (model_list_final_subset[ pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xPRCONS')==True)]) ].max(axis=1) < 0) |
    ...
    model_list_final_subset = model_list_final_subset.loc[
        (model_list_final_subset[ pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xEUR')==True)]) ].min(axis=1) > 0) |
    ...
    model_list_final_subset = model_list_final_subset.loc[
        (model_list_final_subset[ pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xCHF')==True)]) ].min(axis=1) > 0) |
    ...
    model_list_final_subset = model_list_final_subset.loc[
        (model_list_final_subset[ pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xUSD')==True)]) ].min(axis=1) > 0) |
    ...
    model_list_final_subset = model_list_final_subset.loc[
        (model_list_final_subset[ pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xUR')==True)]) ].min(axis=1) > 0) |
    ...
    model_list_final_subset = model_list_final_subset.loc[
        (model_list_final_subset[ pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xCPI')==True)]) ].min(axis=1) > 0) |
    ...
    model_list_final_subset = model_list_final_subset.loc[
        (model_list_final_subset[ pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xHPI')==True)]) ].max(axis=1) < 0) |
    ...
    model_list_final_subset = model_list_final_subset.loc[
        (model_list_final_subset[ pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xIR')==True)]) ].min(axis=1) > 0) |
    ...
    model_list_final_subset = model_list_final_subset.loc[
        (model_list_final_subset[ pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xROBOR')==True)]) ].min(axis=1) > 0) |
    ...
    model_list_final_subset = model_list_final_subset.loc[
        (model_list_final_subset[ pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xEURIBOR')==True)]) ].min(axis=1) > 0) |
    ...
    model_list_final_subset = model_list_final_subset.loc[
        (model_list_final_subset[ pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xCONS')==True)]) ].max(axis=1) < 0) |
        ((model_list_final_subset[ pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xCONS')==True)]) ]).isnull().sum(axis=1) == len( pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xCONS')==True)]) ) ) 
        , :]

    model_list_final_subset = model_list_final_subset.loc[
        (model_list_final_subset[ pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xHPR')==True)]) ].max(axis=1) < 0) |
        ((model_list_final_subset[ pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xHPR')==True)]) ]).isnull().sum(axis=1) == len( pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xHPR')==True)]) ) ) 
        , :]

    model_list_final_subset = model_list_final_subset.loc[
        (model_list_final_subset[ pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xUNE')==True)]) ].min(axis=1) > 0) |
        ((model_list_final_subset[ pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xUNE')==True)]) ]).isnull().sum(axis=1) == len( pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xUNE')==True)]) ) ) 
        , :]

    model_list_final_subset = model_list_final_subset.loc[
        (model_list_final_subset[ pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xIRS1YEMU')==True)]) ].min(axis=1) > 0) |
        ((model_list_final_subset[ pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xIRS1YEMU')==True)]) ]).isnull().sum(axis=1) == len( pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xIRS1YEMU')==True)]) ) ) 
        , :]

    model_list_final_subset = model_list_final_subset.loc[
        (model_list_final_subset[ pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xIRSOV2Y')==True)]) ].min(axis=1) > 0) |
        ((model_list_final_subset[ pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xIRSOV2Y')==True)]) ]).isnull().sum(axis=1) == len( pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xIRSOV2Y')==True)]) ) ) 
        , :]

    model_list_final_subset = model_list_final_subset.loc[
        (model_list_final_subset[ pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xIRS10YEMU')==True)]) ].min(axis=1) > 0) |
        ((model_list_final_subset[ pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xIRS10YEMU')==True)]) ]).isnull().sum(axis=1) == len( pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xIRS10YEMU')==True)]) ) ) 
        , :]


    model_list_final_subset = model_list_final_subset.loc[
        (model_list_final_subset[ pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xGFCF')==True)]) ].max(axis=1) < 0) |
        ((model_list_final_subset[ pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xGFCF')==True)]) ]).isnull().sum(axis=1) == len( pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xGFCF')==True)]) ) ) 
        , :]

    model_list_final_subset = model_list_final_subset.loc[
        (model_list_final_subset[ pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xFBAL')==True)]) ].max(axis=1) < 0) |
        ((model_list_final_subset[ pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xFBAL')==True)]) ]).isnull().sum(axis=1) == len( pd.Series(model_list_final_subset.columns[((model_list_final_subset.columns).str.startswith('xFBAL')==True)]) ) ) 
        , :]
    ...
    #dates before model of residual errors
    Date = dataset_test['Date'].reset_index(drop=True)
    New_Date_BFR_MRE = []
    for date in Date:
        New_Date_BFR_MRE.append(date.strftime('%Y%m%d'))
    New_Date_prefixed_BFR_MRE = ['Y_BFR_MRE_pred_'+char for char in New_Date_BFR_MRE]

    ## dates for after model of residual errors. same as before but needed to double
    Date = dataset_test['Date'].reset_index(drop=True)
    New_Date = []
    for date in Date:
    ...
    model_list_interim[[ "*.comb"+str(o+1)+"_Corr_Pearson" for o in range(no_ind_var) ]] = None # add columns for VIF
    ...
    model_list_interim[[New_Date_prefixed_BFR_MRE]] = None
    model_list_interim[[New_Date_prefixed]] = None
    model_list_interim[['RMSE_BFR_MRE','RMSE']] = None
    ...
    for i in range(0, len(model_list_final_subset)):
        ...
        if(len(comb_corr_pearson)==0):    
            Corr_Pearson=pd.DataFrame({'*.comb1_Corr_Pearson':[0]})
        ...
        new_model = model.get_robustcov_results(cov_type='HAC', maxlags=1)
        ...
        if (model_list_final_subset.iloc[i]['Dependent'])+'_lagged' in (model_list_final_subset.iloc[i]['Independent']):
            ...
            for m in range(1,len(X_test)):
                ...
                Y_pred = pd.DataFrame(model.predict(X_test)).T
            Y_pred.columns = New_Date_BFR_MRE
            Y_pred = Y_pred.add_prefix('Y_BFR_MRE_pred_') # define the dependent variable predicted
        else:    
            X_test = pd.DataFrame(dataset_test[temp_list])
            X_test.insert(loc=0, column='const', value=1) # define the independent variable
        ...
        # calculate residuals
        train_resid = np.array(model.resid)
        #model the training set residuals
        if (len(X_train)>15):
            window = 15
        else: 
            window = ((len(X_train))/2)-1
        #window=20 - rezultate mai slabe iar cu cat maresti apar probleme de estimare, erori
        model_resid = AutoReg(train_resid, lags=window)
        model_resid_fit = model_resid.fit()
        coef_resid = model_resid_fit.params
        # walk forward over time steps in test
        history = train_resid[len(train_resid)-window:]
        history = [history[qq] for qq in range(len(history))]
        predictions = list()
        for t in range(len(Y_pred.T)):
            # persistence
            yhat = np.array(Y_pred.T)[t]
            #error = train_resid[t]  # aici trebuie sa recalculam erorile pentru perioada viitoare (out of time... ) si imi scapa cum
            # predict error
            length = len(history)
            lag = [history[i] for i in range(length-window,length)]
            pred_error = coef_resid[0]
            for d in range(window):
                pred_error += coef_resid[d+1] * lag[window-d-1]
            #correct the prediction
            yhat = yhat + pred_error
            predictions.append(yhat)
            history.append(pred_error)
        #      print('predicted=%f, expected=%f' % (yhat, np.array(Y_pred.T)[t]))
        # # error
        Y_pred_hist = model.predict(X_train)
        RMSE_BFR_MRE_output = pd.DataFrame() 
        RMSE_BFR_MRE_output=RMSE_BFR_MRE_output.append({'RMSE_BFR_MRE':sqrt(mean_squared_error(Y_train, Y_pred_hist))},ignore_index=True)
        RMSE_output = pd.DataFrame()
        RMSE_output=RMSE_output.append({'RMSE':sqrt(mean_squared_error(np.array(Y_pred.T), predictions))},ignore_index=True)


        ##create dataframe for new predictions
        Y_pred_MRE = pd.DataFrame(predictions).T
        Y_pred_MRE.columns = New_Date
        Y_pred_MRE = Y_pred_MRE.add_prefix('Y_pred_') # define the dependent variable predicted

        
        # Concatenate the results for statistic tests
        model_list_temp = pd.concat([VIF_ind, Corr_Pearson, BG1_output, BG2_output, BG3_output, BG4_output, DW_output, SW_output, JB_output, BP_output, WH_output, new_model_pvalue, new_model_var_pvalues, Y_train_prel, Y_pred,Y_pred_MRE,RMSE_BFR_MRE_output,RMSE_output], axis=1)
    ...
    for d in range (1, no_ind_var+1):
        model_list_interim['NW_test_var' + str(d)] = np.where( (model_list_interim['BG1_test']=='Pass') & (model_list_interim['BG2_test']=='Pass') & (model_list_interim['BG3_test']=='Pass') & (model_list_interim['BG4_test']=='Pass') & (model_list_interim['DW_test']=='Pass') & (model_list_interim['BP_test']=='Pass'),'NW not required',
    ...
    header_2 = list(pd.concat( [pd.DataFrame( columns = ['Model_number','Number_of_ind_variables','Dependent',
                            ...
                            pd.DataFrame( columns = list(model_list_final.columns[(model_list_final.columns).str.startswith('Y_BFR_MRE_pred')==True])),
                            pd.DataFrame( columns = list(model_list_final.columns[(model_list_final.columns).str.startswith('Y_pred')==True])),
                            pd.DataFrame( columns = ['RMSE_BFR_MRE','RMSE'])
                            ]))
            ...
            for s in range(2,len(Y_orig.columns)):
                for t in range(5,len(Y_orig)):
                    ...
                    if (suffix[suffix.rfind('_')+1:]=='diff') | (suffix[suffix.rfind('_')+1:]=='diff_diff'):
                        ...
                        Y_orig.iloc[t,s] = 1/Y_orig.iloc[t,s-1]
    ...
    for i in range (0, len(model_list_final)):
        Y_act=pd.DataFrame( model_list_final.loc[i, (list((model_list_final.columns[(model_list_final.columns).str.startswith('Y_act')==True]))) ] )    
        Y_BFR_MRE = pd.DataFrame( model_list_final.loc[i, (list((model_list_final.columns[(model_list_final.columns).str.startswith('Y_BFR_MRE')==True] ))) ] )    
        Y_MRE = pd.DataFrame( model_list_final.loc[i, (list( (model_list_final.columns[(model_list_final.columns).str.startswith('Y_pred')==True] ))) ] )
        Y_BFR_MRE = pd.concat([Y_act,Y_BFR_MRE], axis=0)
        Y_MRE = pd.concat([Y_act,Y_MRE],axis=0)
        dependent_var_name = model_list_final.loc[i,'Dependent']
        Y_BFR_MRE_orig = reverse_y_pred(dependent_var_name, Y_BFR_MRE)  
        Y_orig = reverse_y_pred(dependent_var_name, Y_MRE)  
        Y_BFR_MRE_orig = pd.DataFrame(Y_BFR_MRE_orig.iloc[5:,-1]).T
        Y_orig = pd.DataFrame(Y_orig.iloc[5:,-1]).T
        Y_BFR_MRE_orig.columns = New_Date_BFR_MRE
        Y_orig.columns = New_Date
        Y_BFR_MRE_orig = Y_BFR_MRE_orig.add_prefix('Y_BFR_MRE_orig_')
        Y_orig = Y_orig.add_prefix('Y_orig_')
        Y_BFR_MRE_orig = Y_BFR_MRE_orig.reset_index(drop=True)
        Y_orig = Y_orig.reset_index(drop=True)
        Y_orig=pd.concat([Y_BFR_MRE_orig,Y_orig],axis=1)
    ...
    os.chdir(input_dir)
    r.r.source('R_NW_WH.R')
    os.chdir(str(new_path)) 
    output = r.r['R_BlueTests'](portfolio, hypo, p_value_model, p_value_blue, os.getcwd())
    ...
    independent_variables=('%s, and' %(nr_models, len(df)))
    ...
    criteria_macro=('* no explanatories derived from a single macroeconomic variable')
    f.write('\n%s' %(eliminate_macro))
    f.write('\n%s' %(criteria_macro))

    f.write('\n')
    nr_candidates = ('out of which %d have: ' %(len(model_results)))
    criteria_stats =('* Adjusted R squared greater than 0.3;\n' + '* Model p-value is lower than 0.25;\n' + '* explanatories p-values are lower than 0.25')
    f.write('\n%s' %(nr_candidates))
    f.write('\n%s' %(criteria_stats))
    ...
    model_details_blue_renamed=model_details_not_null_blue.rename(index={'VIF_test':'Multicolinnearity using VIF',
                                                                        ...
                                                                        'R_Intercept_new_pvalue':'p-value of the Intercept after Newey-West','R_NW_test_var1':'Significance after Newey-West for var1 performed in R',
