...
check_list_macro_gdp=[]
check_list_macro_gdp=[('xGDP','xCON'),('xCON','xGDP'),  # gdp with private consumption
...
check_list_macro_house=[]
check_list_macro_house=[('xHPR','xCRE'),('xCRE','xHPR'),  # House prices with Credit
       ('xHPR','xLAN'), ('xLAN','xHPR'), # house prices with land prices
       ('xLAN','xCRE'),('xCRE','xLAN')] # land prices with credit

check_list_macro_rates=[]
check_list_macro_rates=[('xIR','xFX'),('xFX','xIR')]  # Interest Rates with FX
...
try:
    ...
    # Filter independent variables so that we only use the ones that are correlated with the dependent variables
    
    # !!!!!! Possible improvement, add white list with certain independent variables; these variables bypass the filtering
    # Possible further optimization: Memorize all the dependent variables that are correlated with the independent vars; Dictionary: keys = dependent, values = independent
    
    dependent_var = pd.DataFrame(dataset_train.loc[:,dataset_train.columns[pd.Series(dataset_train.columns).str.startswith('x')==False]])
    independent_var = dataset_train.drop(pd.Series(dependent_var.columns), axis=1)
    correlation = pd.DataFrame(columns = dependent_var.columns, index = independent_var.columns)
    for independent in independent_var.columns:
        for dependent in dependent_var.columns:
            corr_coef_current = np.corrcoef(dependent_var.loc[:,dependent],independent_var.loc[:,independent])[1,0]
            correlation.loc[independent, dependent] = corr_coef_current
    
    correlation.to_excel(str(new_path)+"\\"+r'Correlation_' + portfolio + "_" +  hypo + '.xlsx', index = True, header=True)
    
    correlation_test = correlation.apply(lambda row: any(abs(row) >= 0.3), axis=1) # De trecut in ipoteza -> numeric de la 0 la 1 in loc de flag # de adaugat in help info ca 0 = no correlation; greater correlation = faster processing time
    correlated_ind_vars = dataset_train.loc[:, correlation[correlation_test].index]
    dataset_train = pd.concat([dependent_var, correlated_ind_vars], axis=1)
    ...
    for i in range(0,len(df)):
        ...
        for d in range(0,len(comb_full)):
            if len(set(w[0:3] for w in comb_full[d]))==len(comb_full[d]):
